...
**高风险遗漏模式**：项目中存在用户可自定义字段（description、bio、readme 等）经 Markdown 渲染后在 profile 页面输出的代码路径，但没有对应的独立功能点卡片。

**检查方法**：
1. 在 `read-coverage.yaml` 中查找以下关键文件的读取记录：
   - 用户 profile handler 文件（如 `routers/web/user/profile.go`、`routers/web/user/home.go`、`routers/web/feed/profile.go`、`routers/web/shared/user/header.go`）
   - Markdown 渲染函数文件（如 `modules/markup/markdown/markdown.go`）
   - HTML sanitizer 文件（如 `modules/markup/sanitizer.go`）
   - 用户 profile 模板文件（如 `templates/shared/user/profile_big_avatar.tmpl`）

2. 如果上述文件被读取但没有对应的 `user_profile_markdown_rendering` 类型功能点卡片，或现有 markdown 渲染卡片（`FEAT-ri-012-markup`）的 `entry_points` 仅包含 API 端点而缺少 user profile 页面路由，则报告为 **⚠️ 需修复**

3. 检查现有 markdown 渲染卡片的 `code_locations`：
   - 如果只包含 API handler 和 repository 文件渲染路径，缺少 user profile handler 文件和 template 输出文件，则需要为 user profile 渲染创建**独立卡片**
   - 用户 profile 渲染必须包含：user profile handler（`routers/web/user/`）、template 输出文件（`templates/`）、sanitizer 实现文件（`modules/markup/sanitizer.go`）
